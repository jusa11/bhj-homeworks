class Autocomplete {
	...
	getMatches(text) {
		let optionsSelect = Array.from(this.input.options);		// массив опций
		let result = [];

		let words = optionsSelect.filter(elem => elem.textContent.includes(text));   // фильтрую опции, которые подходят под вводимый текст через метод массива опций и метод includes
